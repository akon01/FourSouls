...
export class CardLayout extends cc.Component {
    ...
    _doLayoutHorizontally(baseWidth, applyChildren) {
        ...
        if (this.node.parent.name == 'PlayerItems') {
            ...
            if(this.node.name == "ActiveItems"){
                ...
                layoutY =this.node.convertToNodeSpaceAR( this.node.getPosition()).y - cardHeight + layout.spacingY;
            } else {
                layoutY =this.node.convertToNodeSpaceAR( this.node.getPosition()).y - cardHeight*2; 
            }
    ...
    _addChildrenEventListeners() {
        ...
        for (var i = 0; i < children.length; ++i) {
            var child = children[i];
            child.on(cc.Node.EventType.SIZE_CHANGED, (event) => {
                this._doLayoutDirty('_addChildrenEventListeners');
                ////cc.log('event from: ' +'SIZE_CHANGED1')
            }, this);
            child.on(cc.Node.EventType.POSITION_CHANGED, (event) => { this._doLayoutDirty('_addChildrenEventListeners POSITION_CHANGED'); }, this);
            child.on(cc.Node.EventType.ANCHOR_CHANGED, (event) => { this._doLayoutDirty('_addChildrenEventListeners ANCHOR_CHANGED'); }, this);
            child.on('active-in-hierarchy-changed', (event) => { this._doLayoutDirty('_addChildrenEventListeners active-in-hierarchy-changed'); }, this);
        }
    }
    ...
    _removeChildrenEventListeners() {
        ...
        for (var i = 0; i < children.length; ++i) {
            var child = children[i];
            child.off(cc.Node.EventType.SIZE_CHANGED, (event) => { this._doLayoutDirty('_removeChildrenEventListeners SIZE_CHANGED'); }, this);
            child.off(cc.Node.EventType.POSITION_CHANGED, (event) => { this._doLayoutDirty('_removeChildrenEventListeners POSITION_CHANGED'); }, this);
            child.off(cc.Node.EventType.ANCHOR_CHANGED, (event) => { this._doLayoutDirty('_removeChildrenEventListeners ANCHOR_CHANGED'); }, this);
            child.off('active-in-hierarchy-changed', (event) => { this._doLayoutDirty('_removeChildrenEventListeners active-in-hierarchy-changed'); }, this);
        }
    }
    ...
    _childRemoved(child: cc.Node) {
        child.off(cc.Node.EventType.SIZE_CHANGED, (event) => { this._doLayoutDirty('_childRemoved SIZE_CHANGED'); }, this);
        child.off(cc.Node.EventType.POSITION_CHANGED, (event) => { this._doLayoutDirty('_childRemoved POSITION_CHANGED'); }, this);
        child.off(cc.Node.EventType.ANCHOR_CHANGED, (event) => { this._doLayoutDirty('_childRemoved ANCHOR_CHANGED'); }, this);
        child.off('active-in-hierarchy-changed', (event) => { this._doLayoutDirty('_childRemoved active-in-hierarchy-changed'); }, this);
    ...
    _addEventListeners() {
        ...
        this.node.on('HandCardAdded', (addedChild) => { this._childAdded(addedChild) }, this);
        this.node.on('HandCardRemoved', (removedChild) => { this._childRemoved(removedChild) }, this);
    ...
    _removeEventListeners() {
        ...
        this.node.off(cc.Node.EventType.ANCHOR_CHANGED, (event) => { this._doLayoutDirty('_removeEventListeners ANCHOR_CHANGED'); }, this);
        this.node.off('HandCardAdded', (addedChild) => { this._childAdded(addedChild) }, this);
        this.node.off('HandCardRemoved', (removedChild) => { this._childRemoved(removedChild) }, this);
    ...
    _childAdded(child: cc.Node) {
        ...
        child.on(cc.Node.EventType.ANCHOR_CHANGED, (event) => { this._doLayoutDirty('_childAdded ANCHOR_CHANGED'); }, this);
