...
export default class ButtonManager extends cc.Component {
    ...
    static addNewAddToHandButton(playerNode:cc.Node){
        ...
        switch (playerComp.playerId) {
            case 1:
            buttonNode.setPosition(cc.find('Canvas').convertToNodeSpace(new cc.Vec2(300,300)))             
                break;
            case 2:
                buttonNode.setPosition(cc.find('Canvas').convertToNodeSpace(new cc.Vec2(100,100)))     
                break;        
            default:
                break;
        }
