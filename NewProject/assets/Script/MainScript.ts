...
export default class MainScript extends cc.Component {
    ...
    async onLoad() {
        ...
        if (cc.find('ServerClient') != null) {
            let ServerClient: Server = cc.find('ServerClient').getComponent('ServerClient');
        ...
        var playerManagerComp: PlayerManager = this.playersManager.getComponent('PlayerManager')
        var turnsManagerComp: TurnsManager = this.turnsManager.getComponent('TurnsManager')
        var actionsManagerComp: ActionManager = this.actionsManager.getComponent('ActionManager')
        var buttonsManagerComp: ButtonManager = this.buttonsManager.getComponent('ButtonManager')
        var CardManagerComp: CardManager = this.cardManager.getComponent('CardManager')
        var PilesManagerComp: PileManager = this.pilesManager.getComponent(PileManager)



        //set up store and monster components
        var storeComp: Store = this.store.getComponent(Store)
        var monsterComp: MonsterField = this.monsterField.getComponent(MonsterField)
        ...
        this.scheduleOnce(() => {
            ...
            monsterComp.addMonsterToExsistingPlace(1, CardManager.monsterCardPool.get())
            monsterComp.addMonsterToExsistingPlace(2, CardManager.monsterCardPool.get())
        ...
        //Set up turn lable
        var currentTurnLableComp = cc.find('Canvas').getChildByName('current Turn').getComponent(cc.Label)

        currentTurnLableComp.string = "current turn is:" + TurnsManager.getCurrentTurn().PlayerId;

        //set up player lable
        var currentPlayerLableComp = cc.find('Canvas').getChildByName('current Player').getComponent(cc.Label)
        ...
        MainScript.currentPlayerNode = getCurrentPlayer(PlayerManager.players, TurnsManager.currentTurn)
        MainScript.currentPlayerComp = MainScript.currentPlayerNode.getComponent(Player)
        // ActionManager.updateActions()
