...
export default class Server {
  ...
  bindEvents() {
    ...
    whevent.on(signal.CARD_CHANGE_COUNTER, this.onBroadcastExceptOrigin, this);
    ...
    whevent.on(signal.MARK_DECK_AS_DRAW_FROM_PILE_INSTED, this.onBroadcastExceptOrigin, this);
    ...
    whevent.on(signal.CHOOSE_BUTTON_DATA_COLLECTOR, this.onSendToSpecificPlayer, this);
    whevent.on(signal.CHOOSE_BUTTON_DATA_COLLECTOR_RESPONSE, this.onSendToSpecificPlayer, this);


    whevent.on(signal.MOUSE_CURSOR_MOVE, this.onBroadcastExceptOrigin, this);
  }
  ...
  onMessage(player: ServerPlayer, message: string) {
    try {
      ...
      if (data.signal !== signal.MOUSE_CURSOR_MOVE) {
