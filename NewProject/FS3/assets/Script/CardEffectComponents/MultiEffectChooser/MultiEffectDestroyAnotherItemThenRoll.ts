...
@ccclass('MultiEffectDestroyAnotherItemThenRoll')
export class MultiEffectDestroyAnotherItemThenRoll extends IMultiEffectRollAndCollect {
  collectorName = "MultiEffectDestroyAnotherItemThenRoll";
  ...
  async collectData(data: {
    ...
    const chooseCardComp = new ChooseCard()
    chooseCardComp.chooseType!.chooseType = CHOOSE_CARD_TYPE.ALL_PLAYERS_NON_ETERNAL_ITEMS
    const cardToDestroy = (await chooseCardComp.collectData(data) as EffectTarget).effectTargetCard

    //const thisCard = WrapperProvider.cardManagerWrapper.out.getCardNodeByChild(this.node)
    const cardOwner = WrapperProvider.playerManagerWrapper.out.getPlayerByCard(cardToDestroy)!
    await cardToDestroy.getComponent(Item)!.destroyItem(true)
    return null
  }
