import { Turn, getNextTurn } from "../Modules/TurnsModule";


const { ccclass, property } = cc._decorator;
import { MAX_TURNID, MAX_PLAYERS, CARD_TYPE } from "../Constants"
...
import { chargeCard } from "../Modules/CardModule";
...
export default class TurnsManager extends cc.Component {
   ...
   static nextTurn(recivedEvent:boolean) {
        ...
        cc.find('MainScript').dispatchEvent(new cc.Event.EventCustom('turnChanged', true))
   ...
   static endTurn() {
        if (getNextTurn(TurnsManager.currentTurn, TurnsManager.turns).PlayerId != 0) {
