...
export function endDrag(event: cc.Event.EventTouch) {
    ...
    let newPos = cardNode.parent.convertToNodeSpaceAR(new cc.Vec2(event.getLocationX() - card.xDiff, event.getLocationY() - card.yDiff));
...
export function unCancelDrag(event: cc.Event.EventTouch) {
    let cardNode: cc.Node = event.target;
    let card: Card = cardNode.getComponent('Card')
    let pos = cardNode.parent.convertToNodeSpaceAR(new cc.Vec2(event.getLocationX() - this.xDiff, event.getLocationY() - this.yDiff))
...
export async function cardClickedToActivate(timeToRespondTimeOut,card,data,player){
...
export function chargeCard(card: cc.Node) {
    ...
    switch (card.getComponent(Card).type) {
        case CARD_TYPE.CHAR:
            card.getComponent(Character).activated = false;
            break;
