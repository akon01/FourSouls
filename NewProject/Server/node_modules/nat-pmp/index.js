...
Client.prototype._next = function () {
  console.log(`Client#_next()`);

  debug('Client#_next()');
  var req = this._queue[0];
  if (!req) {
    console.log(`_next: nothing to process`);

    debug('_next: nothing to process');
    return;
  }
  if (!this.listening) {
    console.log(`_next: not "listening" yet, cannot send out request yet`);
  ...
  if (this._reqActive) {
    console.log(`_next: already an active request so waiting...`);
  ...
  console.log('_next: sending request ', buf, gateway);
...
Client.prototype.onlistening = function () {
  console.log('Client#onlistening()');
...
function on(name, target) {
  target.socket.on(name, function () {
    console.log('on: socket event %j', name);
