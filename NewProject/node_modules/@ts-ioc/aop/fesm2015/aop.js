...
let Advisor = class Advisor {
    ...
    add(aspect, raiseContainer) {
        if (!this.aspects.has(aspect)) {
            let metas = core_1.getOwnMethodMetadata(Advice.Advice, aspect);
...
let AdviceMatcher = class AdviceMatcher {
    ...
    match(aspectType, targetType, adviceMetas, target) {
        let aspectMeta = core_1.lang.first(core_1.getOwnTypeMetadata(Aspect.Aspect, aspectType));
        if (aspectMeta) {
    ...
    expressToFunc(type, strExp) {
        if (/^@annotation\(.*\)$/.test(strExp)) {
            ...
            return (name, fullName) => core_1.hasOwnMethodMetadata(annotation, type, name) && !core_1.hasOwnClassMetadata(Aspect.Aspect, type);
        }
        else if (/^execution\(.*\)$/.test(strExp)) {
            let exp = strExp.substring(10, strExp.length - 1);
            if (exp === '*' || exp === '*.*') {
                return (name, fullName) => !!name && !core_1.hasOwnClassMetadata(Aspect.Aspect, type);
            }
...
var D__workspace_github_tsioc_packages_aop_es2015 = createCommonjsModule(function (module, exports) {
Object.defineProperty(exports, "__esModule", { value: true });
...
var index$4 = unwrapExports(D__workspace_github_tsioc_packages_aop_es2015);
